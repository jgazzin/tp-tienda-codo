...
function validar(form) {
    ...
    console.log(email);
    if(email.value === "") {
        alertas(form, 'El email no puede quedar vacío')
        email.classList.add('err')
    } else if (!regex.test(email.value)) {
        alertas(form, "El email no es válido")
    } else {
    ...
    console.log(pass);
    if(pass.value === ''){
        alertas(form, 'Complete una contraseña')
        pass.classList.add('err')
    } else if (pass.value.length < 8){
        alertas(form, 'La contraseña es muy corta');
        pass.classList.add('err')
    } else {
